...
def build_model(identity_len=57, orientation_len=2, lighting_len=4,
    ...
    # @refik added psnr loss. I have tried also 'mse' loss but they seem same even 'msle' is faster.
    model.compile(optimizer=optimizer, loss='msle', metrics=[psnr])
    #model.compile(optimizer=optimizer, loss='msle') #msle is more unstable? (inf)
    #model.optimizer.lr.set_value(0.005) # set learn rate (lower to avoid loss explosion)?
