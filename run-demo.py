...
if __name__ == '__main__':
   ...
   # print("{}".format(all_sequences))
    ...
    if _DEBUG:
        ...
        num_frames = p.processSequence(img_dir_in, img_dir_out, groundtruth_path, _GENERATE_DB=False, _DEBUG=_DEBUG, _LOAD_GT=False)
        end = time.time()
        print("Ended [s]: ", end)
    ...
    if not _DEBUG and False:
        for i, seq in enumerate(frontal_sequences):
            img_dir_in = os.path.join("./in/", seq)
            #img_dir_out = os.path.join("./out/", seq)
            img_dir_out = os.path.join("./out/", "deid-eval-db")
            groundtruth_path = os.path.join("./in/groundtruth/", (seq+'.xml'))
            p.processSequence(img_dir_in, img_dir_out, groundtruth_path, _GENERATE_DB, _DEBUG, False)
